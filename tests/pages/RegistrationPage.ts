...
export class RegistrationPage {
    ...
    constructor(page: Page) {
        ...
        this.usernameErrorMessage = page.locator('#formCover').getByText('Username field is required', { exact: true });
        this.emailErrorMessage = page.locator('#formCover').getByText('Email field is required', { exact: true });
        this.passwordErrorMessage = page.locator('#formCover').getByText('Password field is required', { exact: true });
        this.confirmPasswordErrorMessage = page.locator('#formCover').locator('text=Same as Password');

        // Password requirements
        this.passwordRequirements = ['- Use  4 character or longer ', '- Use maximum 12 character ', '- Including at least one lower letter ', '- Including at least one upper letter ', '- Including at least one number '];
        this.confirmPasswordRequirement = page.locator('#formCover').locator('text=Same as Password');
    ...
    async arePasswordRequirementsVisible(): Promise<boolean> {
        ...
        for (const requirementText of this.passwordRequirements) {
            const requirementLocator = this.page.locator(`#formCover >> text=${requirementText}`);
    ...
    async isUserLoggedIn(): Promise<boolean> {
        try {
            ...
            return userMenuText !== null && userMenuText.trim() !== '' && userMenuText !== 'Hi';
        } catch {
            return false;
        }
    }
